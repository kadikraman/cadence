...
**CRITICAL — Host boundary rule (iOS crash if violated):** Every SwiftUI component (`Text`, `VStack`, `Button`, etc.) **must** be a descendant of a `Host`. A SwiftUI view rendered in a React Native context without a `Host` ancestor will crash the app. This means:

- When building a screen with SwiftUI, wrap the root in `<Host style={{ flex: 1 }}>`.
- When embedding RN views inside SwiftUI, use `RNHostView`. Inside that `RNHostView`, you are back in RN context — any SwiftUI components there need their **own** `Host`.
- **Watch out for cross-platform components** (e.g. from `@/components/universe` or shared abstractions). A component like `PillButton` may render a raw SwiftUI `Button` on iOS without its own `Host`. If you place it inside `RNHostView` (RN context), the SwiftUI view has no `Host` → crash. Always verify what a component renders on iOS before placing it inside `RNHostView`. If it renders SwiftUI, use the native SwiftUI equivalent directly instead.

```tsx
// ❌ CRASH — SwiftUI Button (via PillButton/universe) in RN context, no Host
<Host style={{ flex: 1 }}>
  <VStack>
    <RNHostView matchContents>
      <View>
        <PillButton label="Go" />  {/* universe Button renders SwiftUI Button — no Host! */}
      </View>
    </RNHostView>
  </VStack>
</Host>

// ✅ SAFE — use SwiftUI Button directly (already inside Host → VStack)
<Host style={{ flex: 1 }}>
  <VStack>
    <Button label="Go" modifiers={[buttonStyle('borderedProminent')]} />
  </VStack>
</Host>

// ✅ ALSO SAFE — if you must use a cross-platform component inside RNHostView,
//    wrap it in its own Host
<Host style={{ flex: 1 }}>
  <VStack>
    <RNHostView matchContents>
      <Host matchContents>
...
**Before using any component inside `RNHostView`, always check:** does its iOS implementation (`.ios.tsx`) render SwiftUI components? If yes, it needs a `Host` wrapper or should be replaced with a direct SwiftUI equivalent.
...
| `Link`                   | `destination: string`, `label?: string`, `children?: ReactElement`                                                         | Native SwiftUI link (opens URLs)          |
...
Native SwiftUI [Link](https://developer.apple.com/documentation/swiftui/link) that opens external URLs using the system handler. This is **not** the same as the expo-router `Link` — it opens URLs externally (Safari, universal links, etc.), not in-app routes.

**Props:**

- `destination: string` (required) — The URL to open
- `label?: string` — Simple text label
- `children?: ReactElement` — Custom label content (nested SwiftUI elements only, not plain strings)
- `modifiers` — Standard SwiftUI modifiers array
...
<Link
  label="Open Docs"
  destination="https://docs.expo.dev"
  modifiers={[foregroundStyle('red'), font({ size: 24, weight: 'bold' })]}
/>;
```
...
| `Divider`                       | —                                                                                             | Visual separator   |
| `CircularProgressIndicator`     | `progress?: number\|null`, `color`, `trackColor`, `strokeWidth`, `strokeCap`, `gapSize`       | Circular indicator |
| `LinearProgressIndicator`       | `progress?: number\|null`, `color`, `trackColor`, `strokeCap`, `gapSize`, `drawStopIndicator` | Linear indicator   |
| `CircularWavyProgressIndicator` | `progress?: number\|null`, `color`, `trackColor`                                              | Wavy circular      |
| `LinearWavyProgressIndicator`   | `progress?: number\|null`, `color`, `trackColor`, `stopSize`                                  | Wavy linear        |
...
<BottomSheet isPresented={isOpen} onIsPresentedChange={setIsOpen}>
  <Text
    modifiers={[
      presentationDetents([{ type: 'medium' }, { type: 'large' }]),
...
<CircularProgressIndicator progress={0.7} color="#6200EE" />
<LinearProgressIndicator progress={null} />  {/* indeterminate */}
```
...
<Host style={{ flex: 1 }}>
  ...
  >
    ...
    <Card variant="outlined" modifiers={[fillMaxWidth()]}>
      {/* ... */}
    </Card>
  </LazyColumn>
</Host>
...
### Android: Never use separate Host trees for conditional states

Switching between two different `<Host>` components (e.g. via early return or ternary at the top level) when state changes will crash Android with `IllegalStateException: The specified child already has a parent`. This happens because React unmounts one Host and mounts another, and the Compose runtime cannot cleanly detach/reattach the underlying Android views.

**Always use a single `<Host>` and switch content inside it:**

```tsx
// ❌ CRASH — two separate Hosts, switching on state change
if (items.length === 0) {
  return (
    <Host style={{ flex: 1 }}>
      <Column>
...
return (
  <Host style={{ flex: 1 }}>
    <LazyColumn>{/* content */}</LazyColumn>
  </Host>
);

// ✅ SAFE — single Host, conditional content inside
return (
  <Host style={{ flex: 1 }}>
    {items.length === 0 ? (
      <Column>
...
This also applies to iOS, though it is less likely to crash there. As a general rule: **one screen = one Host**.

### iOS: SwiftUI views inside RNHostView must have their own Host

When you use `RNHostView` to embed RN content inside a SwiftUI tree, you re-enter RN context. Any component placed there that internally renders SwiftUI (e.g. `@/components/universe/Button` renders a raw SwiftUI `Button` on iOS) will crash because there is no `Host` ancestor. This is the most common cause of iOS crashes with `@expo/ui`. See the "Host boundary rule" in the Critical Concept section above for the full rule, safe patterns, and code examples.
...
| Progress          | `ProgressView`                        | `CircularProgressIndicator` / `LinearProgressIndicator` |
